...
using AirThrasher.Assets.Scripts.Enemies;
using AirThrasher.Assets.Scripts.Player;
using AirThrasher;
...
namespace AirThrasher.Assets.Scripts
{
    public class Manager : MonoBehaviour
    {
        ...
        [SerializeField]
        private TMP_Text speedText, scoreText, boostText,
        ...
        [SerializeField] private UserInterface.CircleSpeedIndicator circleSpeed;
        ...
        private void Awake()
        {
            ...
            Player.Player.initializeUI += InitializeUI;
        ...
        void Start()
        {
            ...
            Time.timeScale = 1;
            Player.Player.onFuelUsed += OnFuelUsed;
            Player.Player.onFuelFilled += OnFuelFilled;
            Player.Player.onHealthChanged += PlayerHealthChanged;
            Balloon.balloonPopped += BalloonPopped;
            Player.Player.fratricide += FratricideExplosion;
            Player.Player.engineStateChanged += SwitchStateLights;
            Player.Player.playerHasDied += GameOver;
        ...
        private void InitializeUI(int boost, int health)
        {
            ...
            if (player != null)
            {
                currentPlayer = player.GetComponent<Player.Player>();
        ...
        void Update()
        {
            if (currentPlayer == null) currentPlayer = player.GetComponent<Player.Player>();
        ...
        private void OnFuelUsed(int updatedFuelValue, Player.Player.FuelChangeCause cause)
        {
            switch (cause)
            {
                case Player.Player.FuelChangeCause.Boosted:
                    eventText?.DisplayAnEvent("Boosted", 0);
                    ChangeBoostDisplay(updatedFuelValue);
                    break;
                case Player.Player.FuelChangeCause.FiredGun:
        ...
        private void OnFuelFilled(int updatedFuelValue, Player.Player.FuelChangeCause cause)
        {
            if (energyUsageAnimator.GetCurrentAnimatorStateInfo(0).IsName("Not Displaying"))
            {
                energyUsageAnimator.ResetTrigger("filled");
        ...
        {
            float boostFill = boostFuel / playerMaxBoost;
            boostText.text = boostFuel.ToString();
            boostBar.fillAmount = boostFill;
        }

        private void PlayerHealthChanged(int currentHealth, Player.Player.HealthChangedCause cause)
        {
            switch (cause)
            {
                case Player.Player.HealthChangedCause.EnemyMine:
                    eventText?.DisplayAnEvent("Mine Hit", 1);
                    break;
                case Player.Player.HealthChangedCause.EnemyMissile:
                    eventText?.DisplayAnEvent("Missile Hit", 1);
                    break;
                case Player.Player.HealthChangedCause.EnemyLaser:
                    eventText?.DisplayAnEvent("Laser Hit", 1);
                    break;
                case Player.Player.HealthChangedCause.EnemyRam:
        ...
        private void ChangeHealthDisplay(int healthAmount)
        {
            ...
            float healthFill = healthAmount / (float)playerMaxHealth;
        ...
        private void FratricideExplosion(int scoreIncrease, int updatedFuel, Player.Player.ScoreIncrementCause cause)
        {
            switch (cause)
            {
                case Player.Player.ScoreIncrementCause.FratricideMine:
                    eventText?.DisplayAnEvent("Mine Fratricide", 3);
                    IncreaseScore(scoreIncrease);
                    ChangeBoostDisplay(updatedFuel);
                    break;
                case Player.Player.ScoreIncrementCause.FratricideLaser:
                    eventText?.DisplayAnEvent("Laser Fratricide", 3);
                    IncreaseScore(scoreIncrease);
                    ChangeBoostDisplay(updatedFuel);
                    break;
                case Player.Player.ScoreIncrementCause.FratricideRam:
